...
void setupDxl(uint8_t id)
{
    ...
    // Disable Dynamixel's internal profile (set velocity and acceleration limits to 0 = infinite)
    // This allows the servo to smoothly track the dense interpolated trajectory generated by the host PC 
    // without triggering start-stop behavior on every tiny command update.
